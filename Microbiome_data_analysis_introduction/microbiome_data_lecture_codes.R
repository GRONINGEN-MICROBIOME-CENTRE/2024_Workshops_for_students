...
inDF <- read.table('D:/UMCG/ML_IBD_v5/data_clean/allDataMergedClean_v11.csv',sep=',',header=T,stringsAsFactors = F)
inDF <- inDF[!is.na(inDF$ID),]
inDF2 <- subsetMicrobiomeDF(inDF,getPWYs = F,getVFs = F,getCARDs = F,getPhenos = T,getDivs = F,getTaxa = T)
inDF3 <- filterMetaGenomeDF(inDF2,presPerc = 0.002,minMedRelAb = -1,minMRelAb = -1,rescaleTaxa = T,keepLevels = c("S","G","C","F","O","P","K"),keepUnknown = T)
inDF3 <- inDF3[inDF3$Diagnosis %in% c("CD","HC"),]
inDF3 <- inDF3[inDF3$Cohort %in% c("LLD_IBD"),]
inDF4 <- inDF3[inDF3$ResectionAny =="yes" | inDF3$Diagnosis=="HC",]

inDF4 <- inDF4[!is.na(inDF4$ID),]

inDF4_hc <- inDF4[inDF4$Diagnosis=="HC",][1:10,]
inDF4_ibd <- inDF4[inDF4$Diagnosis=="CD",][1:10,]

inDF5 <- rbind.data.frame(inDF4_hc,inDF4_ibd)
inDF5$Diagnosis[inDF5$Diagnosis=="CD"] <- "Disease"
inDF5$Age <- inDF5$Age+round(rnorm(length(inDF5$Age),mean = 0,sd = 4))
# remove phenos
inDF5$Calprot <- inDF5$Calprot + round(rnorm(length(inDF5$Calprot),mean = 0,sd = 25))
inDF5$Biomarker <- inDF5$Calprot
inDF5$BMI <- inDF5$BMI + round(rnorm(length(inDF5$BMI),mean = 0,sd = 2),digits = 1)

taxaN <- colnames(subsetMicrobiomeDF(inDF5,getPWYs = F,getVFs = F,getTaxa = T,getCARDs = F,getPhenos = F,getDivs = F,getID = F))
inDF6 <- inDF5[colnames(inDF5) %in% c(c("ID","Age","Gender","BMI","Diagnosis","Biomarker"),taxaN)]

# save toy data
# ==================
write.table(inDF6[,colnames(inDF6) %in% c("ID","Age","Gender","BMI","Diagnosis","Biomarker")],file = 'p2_microbiomes_meta.csv',sep=',',row.names = F)
write.table(inDF6[,colnames(inDF6) %in% c("ID",taxaN)],file = 'p2_microbiomes_data.csv',sep=',',row.names = F)
...
# =====================================================================================================================
# =====================================================================================================================
# PART 2: exploratory data analysis, multiple samples
# =====================================================================================================================
# =====================================================================================================================
# load microbiome data: 
inMB <- read.table('p2_microbiomes_data.csv',sep=',',header=T,stringsAsFactors = F)
inMB[,!colnames(inMB)=="ID"] <- inMB[,!colnames(inMB)=="ID"]/100.0
# load metadata: 
...
# III) compare microbiome taxa between healthy and disease
# ======================================================
inDFm2 <- inDFm
