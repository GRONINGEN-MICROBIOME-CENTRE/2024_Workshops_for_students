...
purgeMGNamesOneRow <- function(oneRow) {
  res <- c()
  for (i in oneRow) {
    res <- c(res,purgeMGNameOne(i))
  }
  res
}
...
subsetMicrobiomeDF <- function(inDF,verbose=T,getPWYs=F,getVFs=F,getTaxa=T,getCARDs=F,getPhenos=T,getDivs=F,pwyType="MetaCyc",getID=T,idName="ID",idToRowName=F) {
  ...
  nonPhenoCols <- c()
  nonPhenoColsTaxa <- c()
  if (sum(grepl("UNKNOWN",colnames(inDF)))) {
    nonPhenoColsTaxa <- c(nonPhenoColsTaxa,grep("UNKNOWN",colnames(inDF)))
  }
  if (sum(grepl("UNCLASSIFIED",colnames(inDF)))) {
    nonPhenoColsTaxa <- c(nonPhenoColsTaxa,grep("UNCLASSIFIED",colnames(inDF)))
  }
  if (sum(grepl("^[dgtspcfko]__",colnames(inDF)))) {
    nonPhenoColsTaxa <- c(nonPhenoColsTaxa,grep("^[dgtspcfko]__",colnames(inDF)))
  }
  toGet <- c()
  if (getTaxa) {
...
calcDIVMetrics <- function(inDF,IDcol="RowNames",pwyType="MetaCyc",
  ...
  for (l in DIVlvls) {
    ...
    if (grepl('^tax.$',l)) {
      toUse <- gsub('^tax','',l)
      if (IDcol=="RowNames") {
        inDFf <- subsetMicrobiomeDF(inDF,getTaxa = T,getPhenos = F,getVFs = F,getPWYs = F,getCARDs = F,pwyType = pwyType,
                                    getID = F,idName = "")
      } else {
        inDFf <- subsetMicrobiomeDF(inDF,getTaxa = T,getPhenos = F,getVFs = F,getPWYs = F,getCARDs = F,pwyType = pwyType,
                                  getID = T,idName = IDcol)
      }
      if (IDcol=="RowNames") {
      } else {
      ...
      inDFf <- filterMetaGenomeDF(inDFf,presPerc = filterPres,minMRelAb = filterAbund,minMedRelAb = -1,rescaleTaxa = T,verbose = T,
                                  keepDomains = "All",keepLevels = c(toUse),keepUnknown = F)
  ...
  rownames(DIVMatrix) <- DIVMatrix$RN
  DIVMatrix$RN <- NULL
  return(DIVMatrix)
}
...
calcBCMatrix <- function(inDF,IDcol="RowNames",pwyType="MetaCyc",
  ...
  if (grepl('^tax.$',l)) {
    ...
    if (IDcol=="RowNames" | IDcol=='row.names') {
    } else {
      rownames(inDFf) <- inDFf[[IDcol]]
      inDFf[[IDcol]] <- NULL
    }
...
# ================================================================================
filterMetaGenomeDF <- function(inDF,presPerc = 0.1,
                               minMRelAb = 0.01,
                               minMedRelAb=0.0,
                               rescaleTaxa=T,
                               verbose=T,
  ...
  # -- start -------
  if (verbose) {print('> FILTERING MICROBIOME DATAFRAME:')}
  # alias
  if ('All' %in% keepLevels | 'ALL' %in% keepLevels | 'all' %in% keepLevels) {
    keepLevels=c('T','S','G','F','O','C','P','K')
  }
  # --- deal with unknown ---
  if (!keepUnknown) {
    if (verbose) {print(' >> removing UNKNOWN & UNCLASSIFIED columns')}
    inDF$UNKNOWN <- NULL
    inDF$UNCLASSIFIED <- NULL
    UNKNOWN = rep(0.0,nrow(inDF)) 
  } else {
    if ('UNKNOWN' %in% colnames(inDF)) {
      UNKNOWN <- inDF$UNKNOWN
    } else if ('UNCLASSIFIED' %in%  colnames(inDF)) {
      UNKNOWN <- inDF$UNCLASSIFIED
    } else if ('UNCHARACTERIZED' %in% colnames(inDF)) {
      UNKNOWN <- inDF$UNCHARACTERIZED
    }
  } 
  tCols = grep('^[dgtspcfko]__',colnames(inDF)) # colums with microbiome
  tColsNMG = grep('^[dgtspcfko]__',colnames(inDF),invert = T) # colums with microbiome
  ...
  if (length(keepDomains) == 1) {
    if (keepDomains == 'All' | keepDomains == "") {
      toKeep = grep('k__',colnames(inDF),invert = F)
    } else {
  ...
  if (verbose) {
    clps <-paste(keepDomains,collapse = ', ')
    print (paste0(' > keeping Domains: ',clps,'; ',length(toKeep),' taxa left!')); 
  }  
  inDF <- inDF[,toKeep]
  ...
  # pick kingdoms (K)
  if (length(taxaPCols) > 0) {
    inDF2 <- as.data.frame(inDF2[,taxaPColsKeep])
  }
  ...
  if ('K' %in% keepLevels) {
    if (ncol(taxaK) > 0) {  
      ...
      if (rescaleTaxa) {
          taxaK <- taxaK/(rowSums(taxaK)+UNKNOWN)
        }
      oDF <- cbind(oDF,taxaK)}
  }
  if ('P' %in% keepLevels) {
    if (ncol(taxaP) > 0) {
      ...
      if (rescaleTaxa) {taxaP <- taxaP/(rowSums(taxaP)+UNKNOWN) }
      oDF <- cbind(oDF,taxaP)}
  }
  if ('C' %in% keepLevels) {
    if (ncol(taxaC) > 0) {
      if (verbose) {print(paste(' -> kept',ncol(taxaC),'Classes'))} 
      if (rescaleTaxa) {taxaC <- taxaC/(rowSums(taxaC)+UNKNOWN)}
      oDF <- cbind(oDF,taxaC)}
  }
  if ('O' %in% keepLevels) {
    if (ncol(taxaO) > 0) {
      if (verbose){print(paste(' -> kept',ncol(taxaO),'Orders'))}
      if (rescaleTaxa) {taxaO <- taxaO/(rowSums(taxaO)+UNKNOWN)}
      oDF <- cbind(oDF,taxaO)}
  }
  if ('F' %in% keepLevels) {
    if (ncol(taxaF) > 0) {
      if (verbose){print(paste(' -> kept',ncol(taxaF),'Families'))}
      if (rescaleTaxa) {taxaF <- taxaF/(rowSums(taxaF)+UNKNOWN)}
      oDF <- cbind(oDF,taxaF)}
  }
  if ('G' %in% keepLevels) {
    if (ncol(taxaG) > 0) { 
      if (verbose){ print(paste(' -> kept',ncol(taxaG),'Genera'))}
      if (rescaleTaxa) {taxaG <- taxaG/(rowSums(taxaG)+UNKNOWN)}
      oDF <- cbind(oDF,taxaG)}
  }
  if ('S' %in% keepLevels) {
    if (ncol(taxaS) > 0) {
      if (verbose){print(paste(' -> kept',ncol(taxaS),'Species'))}
      if (rescaleTaxa) {taxaS <- taxaS/(rowSums(taxaS)+UNKNOWN)}
      oDF <- cbind(oDF,taxaS)}
  }
  if ('T' %in% keepLevels) {
    if (ncol(taxaT) > 0) {
      if (verbose){print(paste(' -> kept',ncol(taxaT),'Strains'))}
      if (rescaleTaxa) {taxaT <- taxaT/(rowSums(taxaT)+UNKNOWN)}
      oDF <- cbind(oDF,taxaT)}
  }
  if (keepUnknown) {
    if ('UNKNOWN' %in% colnames(oDF)) {
      if (max(oDF$UNKNOWN) > 1) {
        oDF$UNKNOWN <- oDF$UNKNOWN/100
      }
    } else if ('UNCLASSIFIED' %in%  colnames(oDF)) {
      if (max(oDF$UNCLASSIFIED) > 1) {
        oDF$UNCLASSIFIED <- oDF$UNCLASSIFIED/100
      }
    } else if ('UNCHARACTERIZED' %in% colnames(oDF)) {
      if (max(oDF$UNCHARACTERIZED) > 1) {
        oDF$UNCHARACTERIZED <- oDF$UNCHARACTERIZED/100
      }
    }
  } else {
    oDF$UNKNOWN <- NULL
    oDF$UNCLASSIFIED <- NULL
    oDF$UNCHARACTERIZED <- NULL
  }
  
  if (verbose) {print ('data processing done, returning Dataframe')}
  row.names(oDF) <- oDF$Row.names
  oDF$Row.names <- NULL
  row.names(oDF) <- row.names(inDF)
  oDF
}
# ================================================================================
...
# ========================================================================
# helper function for extracting columns corresponding to given tax level
# ========================================================================
getTaxLevelCols <- function(inData,taxLvl,keepUnknown=T) {
  if (!(taxLvl %in% c("All","K","P","O","C","F","G","S","T"))) {
    exit('ERROR: tax level must be one of [K,P,O,C,F,G,S,T]')
  }
  if (taxLvl == 'K') {
    colsY <- grep('^k__',colnames(inData))
    colsN <- grep('\\|p__',colnames(inData))
  } else if (taxLvl == 'P') {
    colsY <- grep('p__',colnames(inData))
    colsN <- grep('\\|c__',colnames(inData))
  } else if (taxLvl == 'C') {
    colsY <- grep('c__',colnames(inData))
    colsN <- grep('\\|o__',colnames(inData))
  } else if (taxLvl == 'O') {
    colsY <- grep('o__',colnames(inData))
    colsN <- grep('\\|f__',colnames(inData))
  } else if (taxLvl == 'F') {
    colsY <- grep('f__',colnames(inData))
    colsN <- grep('\\|g__',colnames(inData))
  } else if (taxLvl == 'G') {
    colsY <- grep('g__',colnames(inData))
    colsN <- grep('\\|s__',colnames(inData))
  } else if (taxLvl == 'S') {
    colsY <- grep('s__',colnames(inData))
    colsN <- grep('\\|t__',colnames(inData))
  } else if (taxLvl == 'T') {
    colsY <- grep('t__',colnames(inData))
    colsN <- c()
  } else if (taxLvl == 'All') {
    colsY <- grep('^[dgtspcfko]__',colnames(inData))
    colsN <- c()
  }
  cols <- colsY[!colsY %in% colsN]
  if (keepUnknown) {
    if ('UNKNOWN' %in% colnames(inData)) {
      cols <- c(cols,grep('^UNKNOWN$',colnames(inData)))
    } else if ('UNCLASSIFIED' %in%  colnames(inData)) {
      cols <- c(cols,grep('^UNCLASSIFIED$',colnames(inData)))
    } else if ('UNCHARACTERIZED' %in% colnames(inData)) {
      cols <- c(cols,grep('^UNCHARACTERIZED$',colnames(inData)))
    }
  }

  #colnames(inData)[cols]
  return(cols)
}

# ==================================================================================
#  function for getting TOP-X taxa (usually for plotting)
# ==================================================================================
getTopTaxaDF <- function(inData,NR=10,by='Abundance',keepUnknown=T,taxLvl='All',metric='mean') {
  # subsets top X taxa from dataset
  inDF2 <- inData[,getTaxLevelCols(inData= inData,taxLvl = taxLvl,keepUnknown = keepUnknown)]
  if (metric == 'mean') {
    scores <- apply(inDF2,MARGIN = 2,FUN = mean)
  } else if (metric == 'median') {
    scores <- apply(inDF2,MARGIN = 2,FUN = median)
  }
  # sort
  scoress <- sort(scores,decreasing = T,na.last = T)
  if (NR <= length(scoress)) {
    topXnames <- names(scoress[1:NR])
  } else {
    topXnames <- names(scoress)
  }
  inDF2 <- inDF2[,topXnames]
  if (max(inDF2,na.rm = T) > 1) {
    relsum <- 100
  } else {
    relsum <- 1
  }
  inDF2$OTHER <- relsum - rowSums(inDF2,na.rm = T)
}
